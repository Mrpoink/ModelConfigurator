...
def update_dashboard(run_clicks, reset_clicks, prev_clicks, next_clicks, active_tab, color_mode, ghost_mode,
    ...
    if trigger == 'reset-button':
        ...
        state_manager.reset() 
        prompt = ""
        text_out = "System state cleared. All hooks detached."
        ...
        text_out, weights = model_engine.inference(
            prompt, cluster_assignments=current_labels, 
            cluster_magnitudes=c_mags, layer_magnitudes=l_mags
        )
    ...
    if current_emb is not None:
        ...
        for idx in range(len(current_emb)):
            umap_fig.add_scatter(
                ...
                mode='lines', line=dict(color='rgba(0, 255, 65, 0.15)', width=1), showlegend=False, hoverinfo='skip'
            )
        ...
        if color_mode == 'cluster':
            for i in range(num_clusters):
                ...
                umap_fig.add_scatter(
                    x=current_emb[mask, 0], y=current_emb[mask, 1], mode='markers', name=f"Cluster {i}",
            ...
            for i in range(model_engine.num_layers):
                ...
                umap_fig.add_scatter(
                    x=current_emb[mask, 0], y=current_emb[mask, 1], mode='markers', name=f"Layer {i}",
                    text=[h_labels[j] for j, m in enumerate(mask) if m],
    ...
    umap_fig.update_layout(
        ...
        legend=dict(font=dict(color="white", size=13), bgcolor="rgba(20, 20, 20, 0.9)", bordercolor="#00FF41", borderwidth=1, itemsizing="constant"),
        updatemenus=[dict(type="buttons", direction="left", pad={"r": 10, "t": 10}, showactive=True,
            x=1.0, xanchor="right", y=1.1, yanchor="bottom", bgcolor="#222", font=dict(color="#00BB2F", size=11),
            buttons=[dict(label="Hide All", method="restyle", args=[{"visible": "legendonly"}]), dict(label="Show All", method="restyle", args=[{"visible": True}])])]
    )
    heat_fig.update_layout(template="plotly_dark", paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', height=600)

    # --- 5. FINALIZE UI TOGGLES ---
    ...
    cluster_ui = [html.Div([
        ...
        dcc.Slider(id={'type': 'cluster-slider', 'index': i}, min=0, max=2, step=0.01, value=c_mags.get(i, 1.0), marks={0: '0', 1: '1', 2: '2'})
    ...
    layer_ui = [html.Div([
        ...
        dcc.Slider(id={'type': 'layer-slider', 'index': i}, min=0, max=2, step=0.01, value=l_mags.get(i, 1.0), marks={0: '0', 1: '1', 2: '2'})
